...
public class InterfaceKeepaliveSystem {
    ...
    public MonitorService.ResourceCallback mCallback;
    ...
    public void registerCallback(MonitorService.ResourceCallback listener, String pkgname,int pid){
        try{
            mCallback = listener;
    ...
    class SkyResourceCallback extends ISkyResourceCallback.Stub{
        ...
        public void notifyMsg(ResourceMsg resourceMsg) throws RemoteException {
            ...
            if(resourceMsg.what==ResourceMsg.MSG_MONITOR_RESOURCE){
                if(resourceMsg.arg1==ResourceMsg.MSG_MONITOR_RESOURCE_ARG1_SYSTEM_ALL){
                    Bundle bundle = resourceMsg.getData();
                    if(mCallback!=null){
                        mCallback.onSysAll(bundle.getDouble("sysCpuIdleUsage", 0),
                                bundle.getDouble("sysMemAvailableUsage", 0),
                                bundle.getDouble("sysIoWaitUsage", 0));
                    }
                }else if(resourceMsg.arg1==ResourceMsg.MSG_MONITOR_RESOURCE_ARG1_SYSTEM_CPU){
                    Bundle bundle = resourceMsg.getData();
                    if(mCallback!=null){
                        mCallback.onSysCpu(bundle.getFloat("sysCpuIdleUsage", 0));
                    }
                }else if(resourceMsg.arg1==ResourceMsg.MSG_MONITOR_RESOURCE_ARG1_SYSTEM_MEM){
                    Bundle bundle = resourceMsg.getData();
                    if(mCallback!=null){
                        mCallback.onSysMem(bundle.getFloat("sysMemAvailableUsage", 0));
                    }
                }else if(resourceMsg.arg1==ResourceMsg.MSG_MONITOR_RESOURCE_ARG1_SYSTEM_IO){
                    Bundle bundle = resourceMsg.getData();
                    if(mCallback!=null){
                        mCallback.onSysIo(bundle.getFloat("sysIoWaitUsage", 0));
                    }
                }
            }
        }
